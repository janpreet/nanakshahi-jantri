...
export class Engine {
  // new Engine()                       — bundled Jantri data
  // new Engine('/path/to/data')        — alternate data directory
  // new Engine({ dataDir, overrides }) — overrides: object or path to a JSON file
  constructor(opts = {}) {
    if (typeof opts === 'string') opts = { dataDir: opts };
    ...
    let overrides = opts.overrides ?? (existsSync(join(dataDir, 'overrides.json'))
      ? join(dataDir, 'overrides.json') : null);
    if (typeof overrides === 'string') overrides = readJSON(overrides);
    this.overrides = overrides ?? { add: [], hide: [], rename: {} };
